...
from data import make_ccl_loader, make_pesudo_loader, make_mpesudo_loader
...
def test(model, test_loader, thresh=0.5, save_pth='./temp_res_test.txt', save_multi_pth="./temp_res_multi_test.txt", label_path="./"):
    model.eval()
    res_txt = open(save_pth, 'w')
    res_mul_txt = open(save_multi_pth, 'w')
    # res_dic = {}
    with torch.no_grad():
        for i,(img,_,_,img_pth) in tqdm(enumerate(test_loader)):
            if torch.cuda.is_available():
                img = img.cuda()
                model = model.cuda()
            # score, score0 = model(img,img)
            score, score_multi = model(img, None, isPair=False)

            pred_logit = torch.nn.Softmax(dim=1)(score)
            pred_multi_logit = torch.nn.Softmax(dim=1)(score_multi)
            ...
            for pth, logit in zip(img_pth, pred_multi_logit.cpu().numpy()):
                logit = str("{:.5f} {:.5f} {:.5f}".format(logit[0], logit[1], logit[2]))
                pic_name = pth.split('/')[-1]
                res_mul_txt.write(pic_name+' '+logit+'\n')
...
def do_train(cfg, model0,model1, train_loader, val_loader, test_loader, pesudo_loader, optimizer, scheduler, loss_func,loss_constra,logger):
    ...
    # loss_maes = AverageMeter()
        ...
        (img,plabel,plabel0,_,_))  in enumerate(zip(train_loader, pesudo_loader)):
            # print(label,maskid)
            # print(img.shape)
            ...
            if torch.cuda.is_available():
                ...
                img = img.cuda()
                plabel = plabel.long().cuda()
                plabel0 = plabel0.long().cuda()
            ...
            loss_pesudo = loss_cls(scorep, plabel) + \
                        loss_cls(score_maskp, plabel0)
        ...
        if ((epoch+1) % cfg.SOLVER.PESUDO_UPDATE_PERIOD == 0) and ((epoch+1) >= cfg.SOLVER.PESUDO_SKIP):
            ...
            pesudo_multi_pth = os.path.join(pesudo_dir, "pesudo_scores_multi_{}.txt".format(epoch))

            test(model1,test_loader, save_pth=pesudo_pth, save_multi_pth=pesudo_multi_pth, thresh=0.5, label_path=None)
            pesudo_loader.dataset.update_labels(data_scores=pesudo_multi_pth, thresh=0.9)
        ...
        if (epoch+1) % cfg.SOLVER.EVAL_PERIOD==0:
            ...
            if ((epoch+1) % cfg.SOLVER.PESUDO_UPDATE_PERIOD == 0) and ((epoch+1) >= cfg.SOLVER.PESUDO_SKIP):
                ...
                test(model1,test_loader, save_pth=os.path.join(test_dir, "temp_res_test_{}.txt".format(epoch)), 
                save_multi_pth=os.path.join(test_dir, "temp_res_test_multi_{}.txt".format(epoch)),thresh=0.5, label_path=None)
...
def train(cfg,logger):
    ...
    pesudo_loader = make_mpesudo_loader(cfg)
    # print(num_classes)
    # prepare model
    ...
    if cfg.MODEL.PRETRAIN_CHOICE == 'self':
        ...
        model0.load_param(cfg.MODEL.PRETRAIN_PATH)
