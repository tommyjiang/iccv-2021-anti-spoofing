...
class Test_mask(BaseImageDataset):
    """
    ...
    def _process_val(self, dir_path="../../test",raw_data_pth='../raw_data/phase1/val'):
        ...
        for folder in img_folder:
            # cur_mask = 1
            # print(folder)
            cur_folder_pth = os.path.join(dir_path,folder)
            for img_name in os.listdir(cur_folder_pth):
                img_pth = os.path.join(cur_folder_pth,img_name)
                dataset.append((img_pth,cur_pid,cur_label,cur_mask,points_list))
        return dataset
    def _process_test(self, dir_path="../extra_data/pts_v2/test",raw_data_pth='../../test',val_label = './val.txt'):
