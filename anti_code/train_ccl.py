...
def val(model, val_loader, save_pth='./', label_path="./"):
    model.eval()
    ...
    res_txt.close()
    thresh,apcer,bpcer,acer = eval_res(os.path.join(save_pth, "temp_res_val.txt"), label_path, eval_val="heihei")
    return thresh,apcer,bpcer,acer

def test(model, test_loader, thresh=0.5, save_pth='./', label_path="./"):
    model.eval()
    # model_name = cfg.TEST.WEIGHT.split('/')[-2]+'_'+cfg.TEST.WEIGHT.split('/')[-1].split('.')[0]
...
def do_train(cfg, model0,model1, train_loader, val_loader, test_loader, optimizer, scheduler, loss_func,loss_constra,logger):
    # losses = AverageMeter()
    ...
    # loss_maes = AverageMeter()
        ...
        for i, (img0,img1,pair_label,label0,label1,maskid0,maskid1) in enumerate(train_loader):
            ...
            # 添加三分类分支监督
            loss_data =loss_ema_m + loss_func(score_mask0,maskid0,score_mask1,maskid1)+loss_func(score0,label0,score1,label1)+0.7*(loss_constra(feat0,feat1_ema, target_data)+loss_constra(feat0_ema,feat1, target_data)) / 2
            # loss_mse = torch.nn.MSELoss()
        ...
        if (epoch+1) % cfg.SOLVER.EVAL_PERIOD==0:
            thresh, apcer,bpcer,acer = val(model1,val_loader, save_pth=cfg.OUTPUT_DIR, label_path="../extra_data/labels/val.txt")
            logger.info("VALID APCER:{:.4f} BPCER:{:.4f} ACER:{:.4f}".format( apcer*100, bpcer*100, acer*100))
            test(model1,test_loader, save_pth=cfg.OUTPUT_DIR, thresh=thresh, label_path=None)
